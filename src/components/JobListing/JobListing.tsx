...
export function JobListing({ jobsData, addFilter }: Props) {
  const jobListingsMarkup = jobsData.map((job, index) => {
    ...
    return (
      <article
        className="p-4 pt-0 md:pt-4 text-left rounded-md shadow-xl shadow-desaturated-dark-cyan/30 bg-white md:flex md:items-center md:justify-center md:gap-5"
        key={job.id}
      >
        <img
          src={`../assets/images/${job.logo}`}
          alt="logo"
          className="w-12 mt-[-25px] md:mt-0 md:w-[80px] md:h-[80px]"
        />
        <div>
